namespace DC_Font_Generator
{
    ...
    {
        //DC - Double byte Character
        //SC - Single byte Character
        #region Members
        ...
        public MainForm()
        {
            ...
            LangSetup();
            this.Text = string.Format("{0} {1} [Version: {2}]", base.ProductName,GetString("by aabby"), base.ProductVersion);
            ...
            if (GamePath != "")
            {
                if (Directory.Exists(GamePath))
                {
                    ...
                    if (!Directory.Exists(GamePath))
                    { Directory.CreateDirectory(FontPath); } /*Создание папки Шрифты*/
                }
                else { GamePath = ""; }
            }
            if (GamePath == "")
            ...
            if ((Encoding.Default.EncodingName == "Кириллица (windows)") || (Encoding.Default.BodyName == "koi8-r"))
            {
                Encoding_comboBox.SelectedIndex = 0;
            }
        ...
        private void SetNowData()
        {
            ...
            if (MainList[MainSelect].Fallout3INI.Count > 0) { /* Do nothing*/ }
            for (int i = 0; i < 8; i++)
            {
            ...
            {
                 for (int i = 0; i < 8; i++)
                     if (MainList[index].Fallout3INI[i]) { INIstat[i] = index; }
                     index++;
            }
            for (int i = 0; i < 8; i++)
            {
        ...
        private string GetGamePath()
        {
            ...
            List<string> game = new List<string>();
                game.Add("Fallout3");
                game.Add("FalloutNV");
            theGame = game[1];// Говнокод плодим ! ;)
            // Auslesen des Fallout3-Installationsordners aus der Registry
            RegistryKey userKey = Registry.LocalMachine;
            try
            {
                userKey = userKey.OpenSubKey(@"SOFTWARE\Bethesda Softworks\", false);
                if (userKey != null)
                {
                    userKey.OpenSubKey(theGame);
                ...
                {
                    userKey = userKey.OpenSubKey(@"SOFTWARE\Wow6432Node\Bethesda Softworks\", false);
                    if (userKey != null)
                    {
                        userKey.OpenSubKey(theGame);
                        path = userKey.GetValue("Installed Path").ToString();
        ...
        /// 檢查Fnt資料
        /// </summary>
        /// <remarks>Идет проверка на длинну файла!! </remarks>
        ...
        /// 選擇字型 | Font Select
        /// </summary>
        /// <remarks>Сменить метод на вызов ф-и с которой выбран текущий шрифт</remarks>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void label_Click(object sender, EventArgs e)
        {
            FontDialog fd = new FontDialog();
            #region fd Properties
            fd.ShowColor = false;
            ...
            switch (Tag)//если название label..
            {//выставляем стандартный шрифт.
                case ("Font1"):
            ...
            try
            {
                ...
                {
                    Font font = new Font(fd.Font.FontFamily, fd.Font.SizeInPoints, fd.Font.Style, GraphicsUnit.Pixel);

                    ((Label)sender).Text = font.Name + "," + font.Size;
                    switch (Tag)
                    {
                        case ("Font1"):
        ...
        private void sOpenFNT_Click(object sender, EventArgs e)
        {
            FontDialog fd = new FontDialog();
            #region fd Properties
            ...
            try
            {
                if (fd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if(Encoding_comboBox.SelectedIndex == 0)
                    { 
                            label2.Text = font.Name + "," + font.Size;
                            this.MainList[MainSelect].font1 = font;
                            this.MainList[MainSelect].ImportFont1name = "";
                    }
                    else if(Encoding_comboBox.SelectedIndex >0)
                    {
                            label4.Text = font.Name + "," + font.Size;
                            this.MainList[MainSelect].font2 = font;
                            this.MainList[MainSelect].ImportFont2name = "";
                            this.MainList[MainSelect].DCfontLink = -1;
                    }
        ...
        private void checkBox_fixed_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (checkBox_fixed.Checked) { if (ready) this.MainList[MainSelect].DrawMode = 0; }
            else { if (ready) this.MainList[MainSelect].DrawMode = 1; }
        ...
        private void buttonOpenFNT_Click(object sender, EventArgs e)
        {
            ...
            if (!_ImportFntAndTex(path, filename))
            {
            ...
            if (ChangeImageSize()) { StatusText = GetString("Open fnt and tex done."); }
            else { StatusText = GetString("file size error."); }
        }
        private bool _ImportFntAndTex(string path, string filename)
        {
            ...
            foreach (Fnt_char fnt in this.MainList[MainSelect].FntFile.CharList)
            {
                if (fnt.Enable)
                {
                    if (fenc.IsBand(fnt.HEX)) { fnt.Enable = false; }
                }
            }
        ...
        private void OPENttf()
        {
            #region properties
            openFileDialog1.DefaultExt = "ttf";
            openFileDialog1.Title = "Open a True Type Font file";
            openFileDialog1.FileName = "";
            openFileDialog1.Filter = ".ttf|*.TTF";
            openFileDialog1.FilterIndex = 1;
            openFileDialog1.InitialDirectory = FontPath;
            #endregion
            if (this.openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                string filename = Path.GetFileNameWithoutExtension(this.openFileDialog1.FileName);
                string path = Path.GetDirectoryName(this.openFileDialog1.FileName) + @"\" + filename + ext;
                ////////////////////////////////////////////////////////////////////////////////////////////
                System.Drawing.Text.PrivateFontCollection getFNT = null;
                SettsFnt sf = null;
                //lock (getFNT)
                //{
                    getFNT = new System.Drawing.Text.PrivateFontCollection();
                    if (ext == ".ttf") { getFNT.AddFontFile(path); }
                    sf = new SettsFnt();
                    sf.getFNT = getFNT;

                    /* Тут вызов диалога настройки файла шрифта, передача параметров между формой и т.д.
                     * параметры:
                                    Стиль Начертания : обычный, курсив, полужирный, полужирный курсив
                     *              Размер: 8-72 (поиск по регулярке)
                     *              Образец: АаБб 01 
                     */
                    //Font fontX = new Font(ff, /*размер шрифта float*/, /*Стиль начертания*/, GraphicsUnit.Pixel);
                    ...
                    if (sf.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        fsy = sf.fsy;
                        if (emSize == 0) return;
                        Font fontX = new Font(getFNT.Families[0], emSize, fsy, GraphicsUnit.Point);

                        label2.Text = fontX.Name + "," + fontX.Size;
                        this.MainList[MainSelect].font1 = fontX;
                        this.MainList[MainSelect].ImportFont1name = ""; //зачем это нужно, кстати?

                        this.MainList[MainSelect].Clear(); //清除已經繪製的字 | Remove the word that has been drawn
                        this.button1.Enabled = false; //Save font

                        label2.Font = fontX;
                        if (fontX.Size > 27)
                        {//больше 28 не будет для label
                            var fontN = new Font(getFNT.Families[0], 28f, sf.fsy, GraphicsUnit.Pixel);
                            label2.Font = fontN;
                        } 
                        //Render();
                    }
        ...
        {
            if (!this.TexEnable) { return; }

            StatusText = GetString("Please wait...");
            if (this.MainList.Count > 1) { this.saveFileDialog1.Title = "Save Tex"; }
            else { this.saveFileDialog1.Title = "Save Fnt and Tex"; }
        ...
        /// <summary>
        /// Выполняет отрисовку смволов на PictureBox
        /// </summary>
        private void Render()
        {
            //移除RemoveFnt
            ...
            MakeFonts(); //製造字元 | Производство символов

            DrawFonts(); //繪製文字 | Рисование текста

            //關聯文字複製 | авто линковка, если режым для япно.
            foreach (Main m in MainList)
            {
                m.LinkClone();
            }
        ...
        private void Encoding_comboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (index >= 0)
            {
                if (index == 0)//dc選取字型 | Select the font type = dc
                { label4.Enabled = false; }
                else
                { label4.Enabled = true; }
                this.sOpenFNT.Enabled = true;
                this.button1.Enabled = false;
                ...
                this.EnableLink(); //更動字型Link狀態 | Link state change font
                //toolStripProgressBar1.Visible = false;
        ...
        private void INI_Font_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (key != "") { ini.IniWriteValue("Fonts", key, ff.intFontPath); }
        }
        /// <summary>
        ...
        public string MouseLeftClick(int x, int y,bool selected,bool remove)
        {
            ...
            string s = string.Format(this.ToolTipFormat, c, hexOutput, fnt.charViewWidth, fnt.charViewHeight, fnt.BottomAlign, fnt.LeftSpace, fnt.RightSpace, bx - rx, by - ry, fnt.ID);
        ...
        public void MaskReset()
        {
            ...
            mask.TextRenderingHint = System.Drawing.Text.TextRenderingHint.SingleBitPerPixel;
                ...
                case ("-"):
                    if (this.MainList.Count > 0)
                    {
                        ...
                        foreach (Main m in this.MainList)
                        {
                            ...
                            {
                                m.ID--;
                                foreach (Fnt_char fnt in m.FntFile.CharList) { fnt.ID = m.ID; }
                            }
        ...
        private void textBoxInputText_TextChanged(object sender, EventArgs e)
        {
            ...
            if (Text != "")
            {
                ...
                if (b.Length == 1) { hexOutput = String.Format("{0:X2}", b[0]); }
                else if (b.Length > 1) { hexOutput = String.Format("{0:X2}{1:X2}", b[0], b[1]); }
            }            
            ready = false;
            switch (Tag)
            {
                    ...
                    switch (myReader.NodeType)
                    {
                        case XmlNodeType.Element:
                            ...
                            switch (LocalName)
                            {
                                ...
                                case("import_font"):
                                    myReader.Read(); value = myReader.Value;
                                    if (!_ImportFntAndTex(Path.Combine(FontPath, value + ".fnt"), value))
                                    {
